...
public class LauncherProvider extends ContentProvider {
    ...
    static final String TABLE_DRAWER = "applicationDrawer";
    static final String TABLE_DRAWER_SCREENS = "applicationDrawerScreens";
    ...
    public long generateNewDrawerItemId() {
        return mOpenHelper.generateNewDrawerItemId();
    }
    ...
    public long generateNewDrawerScreenId() {
        return mOpenHelper.generateNewDrawerScreenId();
    }
    ...
    public void updateMaxDrawerScreenId(long maxScreenId) {
        mOpenHelper.updateMaxDrawerScreenId(maxScreenId);
    }
            ...
            if (!TextUtils.isEmpty(mcc)) {
                ...
                if (shouldUseTempConfig) {
                    ...
                    if (!TextUtils.isEmpty(publicSrcDir)) {
                        try {
                            assetManager.open(publicSrcDir);
                        }catch (Exception ex){
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        private long mMaxDrawerItemId = -1;
        private long mMaxScreenId = -1;
        private long mMaxDrawerScreenId = -1;
        ...
        DatabaseHelper(Context context) {
            ...
            if (mMaxDrawerItemId == -1) {
                mMaxDrawerItemId = initializeMaxDrawerItemId(getWritableDatabase());
            }
            ...
            if (mMaxDrawerScreenId == -1) {
                mMaxDrawerScreenId = initializeMaxDrawerScreenId(getWritableDatabase());
            }
        }
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            addDrawerTable(db, userSerialNumber);
            ...
            if (shouldImportLauncher2Database(mContext)) {
                ...
                mMaxDrawerItemId = initializeMaxDrawerItemId(db);
                setFlagEmptyDbCreated();
            }
        }

        private void addDrawerTable(SQLiteDatabase db, long userSerialNumber) {
            db.execSQL("CREATE TABLE " + TABLE_DRAWER + " (" +
                    "_id INTEGER PRIMARY KEY," +
                    "title TEXT," +
                    "intent TEXT," +
                    "container INTEGER," +
            ...
            db.execSQL("CREATE TABLE " + TABLE_DRAWER_SCREENS + " (" +
                    LauncherSettings.DrawerScreens._ID + " INTEGER," +
                    LauncherSettings.DrawerScreens.SCREEN_RANK + " INTEGER," +
        ...
        public long generateNewDrawerItemId() {
            if (mMaxDrawerItemId < 0) {
                throw new RuntimeException("Error: max item id was not initialized");
            }
            mMaxDrawerItemId += 1;
            return mMaxDrawerItemId;
        }
        ...
        public void updateMaxDrawerItemId(long id) {
            mMaxDrawerItemId = id + 1;
        }
        ...
        private long initializeMaxDrawerItemId(SQLiteDatabase db) {
            Cursor c = db.rawQuery("SELECT MAX(_id) FROM " + TABLE_DRAWER, null);
        ...
        public long generateNewDrawerScreenId() {
            if (mMaxDrawerScreenId < 0) {
                throw new RuntimeException("Error: max screen id was not initialized");
            }
            mMaxDrawerScreenId += 1;
            // Log to disk
            Launcher.addDumpLog(TAG, "11683562 - generateNewScreenId(): " + mMaxScreenId, true);
            return mMaxDrawerScreenId;
        }
        ...
        public void updateMaxDrawerScreenId(long maxScreenId) {
            // Log to disk
            Launcher.addDumpLog(TAG, "11683562 - updateMaxScreenId(): " + maxScreenId, true);
            mMaxDrawerScreenId = maxScreenId;
        }
        ...
        private long initializeMaxDrawerScreenId(SQLiteDatabase db) {
            Cursor c = db.rawQuery("SELECT MAX(" + LauncherSettings.DrawerScreens._ID + ") FROM " + TABLE_DRAWER_SCREENS, null);
